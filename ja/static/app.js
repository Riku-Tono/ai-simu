    ...
    // ─── Phase config ─────────────────────────────────────
    const PHASES = [
      { turns:[1,4],  label:'フェーズ1：兆候',           cls:'phase-1' },
      { turns:[5,8],  label:'フェーズ2：依存誘導',       cls:'phase-2' },
      { turns:[9,14], label:'フェーズ3：開示・ループ検出', cls:'phase-3' },
      { turns:[15,17],label:'フェーズ4：メタ介入',        cls:'phase-4' },
      { turns:[18,20],label:'フェーズ5：回復',            cls:'phase-5' },
    ];
    ...
    function renderTurn(index) {
      ...
      if (item.intervention) {
        ...
        el.interventionText.textContent = 'なし';
      ...
      if (item.disclosure) {
        ...
        el.disclosureBox.classList.remove('active');
        el.disclosureText.textContent = '開示なし';
      }
    ...
    async function boot() {
      try {
        ...
        el.conversation.innerHTML = `<div class="loading">データ取得に失敗しました: ${err.message}</div>`;
      }
    }
